...
`/api/jobs/purge` deletes mail older than thirty days, abuse counters
older than a day, and deletion requests handled more than a year ago. `vercel.ts` runs it daily and Vercel sends `CRON_SECRET`
as a bearer token.
